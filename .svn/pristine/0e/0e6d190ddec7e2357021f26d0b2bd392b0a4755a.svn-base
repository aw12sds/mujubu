...
namespace mujubu.工艺
{
    public partial class 增加工序1 : Form
    {
        ...
        {
            string 插入顺序 = textEdit4.Text.Trim();
            double b = 0.0;
            int a = 0;
            ...
            if (int.TryParse(textEdit1.Text, out a) == false)
            {
                MessageBox.Show("数量必须是数字！");
                return;
            }
            if (Double.TryParse(textEdit2.Text, out b) == false)
            {
            ...
            string sql1 = "INSERT INTO tb_gongxu_manage(零件id,工序名称,工序内容,操作人,加工数量,金额单价,顺序) VALUES('" + id + "', '" + cbGongxu.Text + "','" + memoEdit1.Text + "','" + comboBoxEdit2.Text + "','" + textEdit1.Text.Trim() + "','" + textEdit2.Text.Trim() + "','" + 插入顺序 + "')";
